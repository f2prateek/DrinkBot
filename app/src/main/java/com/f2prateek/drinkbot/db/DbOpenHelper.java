...
final class DbOpenHelper extends SQLiteOpenHelper {
  ...
  @Override public void onCreate(SQLiteDatabase db) {
    ...
    long groceryListId =
        db.insert(TodoList.TABLE, null, new TodoList.Builder().name("Grocery List").build());
    db.insert(TodoItem.TABLE, null,
        new TodoItem.Builder().listId(groceryListId).description("Beer").build());
    db.insert(TodoItem.TABLE, null,
        new TodoItem.Builder().listId(groceryListId).description("Point Break on DVD").build());
    db.insert(TodoItem.TABLE, null,
        new TodoItem.Builder().listId(groceryListId).description("Bad Boys 2 on DVD").build());

    long holidayPresentsListId =
        db.insert(TodoList.TABLE, null, new TodoList.Builder().name("Holiday Presents").build());
    db.insert(TodoItem.TABLE, null, new TodoItem.Builder().listId(holidayPresentsListId)
    ...
    long workListId =
        db.insert(TodoList.TABLE, null, new TodoList.Builder().name("Work Items").build());
    db.insert(TodoItem.TABLE, null, new TodoItem.Builder().listId(workListId)
    ...
    long birthdayPresentsListId = db.insert(TodoList.TABLE, null,
        new TodoList.Builder().name("Birthday Presents").archived(true).build());
